export const CHURCH_PAYMENTS = ['文化幣', 'LINE PAY', '現金'];
...
export function formatOrderNumber(tenant, day, sequence, organizer = 'church') {
  return (
    (organizer === 'bookstore' || !tenant ? 'PF' : tenant.toUpperCase()) +
    day.replaceAll('-', '') +
...
export function validateRoleChange(role, tenant, patch, current) {
  ...
  if (
    role !== 'admin' &&
    JSON.stringify(order.paymentRecords) !==
      JSON.stringify(current?.paymentRecords) &&
